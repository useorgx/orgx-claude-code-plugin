...
export function createOrgXClient({ apiKey, baseUrl, userId }) {
  ...
  return {
    ...
    // false-completion / hallucinated-receipt / dependency-violation signals
    // and surfaces them on /live. Structured sibling of emitActivity.
    async emitExecutionGraph(payload) {
      const response = await request("POST", "/api/client/live/execution-graph", payload);
      if (response && typeof response === "object" && response.data) return response.data;
      return response;
    },
    ...
    async getRelevantLearnings(payload) {
      const response = await request("POST", "/api/client/learnings/relevant", payload);
      if (response && typeof response === "object" && response.data) return response.data;
      return response;
    },

    async classifyTaskModel(payload) {
      const response = await request("POST", "/api/client/classify-model", payload);
      if (response && typeof response === "object" && response.data) return response.data;
      return response;
    },
  };
