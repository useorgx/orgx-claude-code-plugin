 ...
 * @useorgx/orgx-gateway-sdk and manages the weekly license heartbeat.
 *
...
export async function startPeer(opts) {
  ...
  const client = new PeerClient({
    ...
    drivers: [driver],
    onOpen: () => console.log('[orgx-claude-code-plugin] connected'),
    onClose: (code, reason) => console.warn('[orgx-claude-code-plugin] closed', { code, reason }),
  ...
  if (!opts.skipHeartbeat) {
    await postHeartbeat(baseUrl, opts, manifest).catch((err) =>
      console.warn('[orgx-claude-code-plugin] initial heartbeat failed', err)
    );
    heartbeatTimer = setInterval(() => {
      postHeartbeat(baseUrl, opts, manifest).catch((err) =>
        console.warn('[orgx-claude-code-plugin] weekly heartbeat failed', err)
      );
    }, HEARTBEAT_MS);
